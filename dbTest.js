...
async function testDb() {
    try {
        ...
        console.log(`Searching for user with the email ${testEmail}`);
        let user = await db.user.findOne({ where: { email: testEmail } });

        if (user) {
            console.log(`Found user with the email ${testEmail}`);
        } else {
            console.log(`No user found with the email ${testEmail}. Attempting to create user...`);
            user = await db.user.create({
                name: 'Test User',
                email: testEmail,
                password: 'testPassword'
            });
            console.log(`User created with the email ${testEmail}`);

            user = await db.user.findOne({ where: { email: testEmail } });
            if (user) {
                console.log(`Confirmed that user with the email ${testEmail} exists in the database`);
            } else {
                console.log(`Unable to find user with the email ${testEmail} after creation`);
            }
        }
    } catch (error) {
